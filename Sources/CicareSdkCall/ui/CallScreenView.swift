...
public class CallScreenViewController: UIViewController {
    ...
    var calleeName: String = "John Doe"
    var callStatus: String = "Connecting..."
    var avatarUrl: String? = "https://avatar.iran.liara.run/public/23"
    ...
    @objc private func handleNetworkSignal(_ notification: Notification) {
        guard let signalStrengthString = notification.userInfo?["signalStrength"] as? String else { return }
        if (isConnected) {
            DispatchQueue.main.async {
                switch signalStrengthString {
                case "weak":
    ...
    @objc private func handleCallStatus(_ notification: Notification) {
        ...
        print("status call \(statusString)")
        DispatchQueue.main.async {
            switch status {
            case .incoming:
